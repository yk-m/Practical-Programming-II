...
public class NonlinearEquation extends Applet {
	...
	private Answer _solveNLEByRegulaFalsi( double initialValue, Equation e ) {
		double current // value
		     , previous = initialValue // pastValue
		     , lowerEnd = NEGATIVE_MAX
		     , upperEnd = POSITIVE_MAX
	...
	public void paint( Graphics g ){
		...
		initAxis( new Coordinate( -1, 0.5 ), new Coordinate( 5, -0.3 ), 1, 0.1 );
		drawEquationInRange( new A102(), NEGATIVE_MAX, POSITIVE_MAX, Color.red, Color.blue );

		try {
			// 値２つ返したかったのでAnswerクラスを作りました
			// 他の方程式でやりたくなったときのためにクラスを分けました
			Answer a = this._solveNLEByRegulaFalsi( 0, new A102() );
