...
public class NonlinearEquation extends Applet {
	...
	private Answer _solveNLEByNewton( double initialValue, Equation e ) {
		double current // value
		     , previous = initialValue // pastValue
		;
		...
		drawLine( new Coordinate( initialValue, 0 ), new Coordinate( initialValue, e.f( initialValue ) ), Color.green );

		for( int i = 0; i <= MAXIMUM_IT; i++ ) {
			current = previous - e.f( previous )/e.df( previous );
			drawLine( new Coordinate( current, 0 ), new Coordinate( current, e.f( current ) ), Color.orange );
			drawLine( new Coordinate( current, 0 ), new Coordinate( previous, e.f( previous ) ), Color.red );
	...
	public void paint( Graphics g ){
		...
		initAxis( new Coordinate( -0.5, 1.5 ), new Coordinate( 2, -0.3 ), 0.1, 0.1 );
		drawEquation( new A111(), Color.blue );

		try {
			// 値２つ返したかったのでAnswerクラスを作りました
			// 他の方程式でやりたくなったときのためにクラスを分けました
			Answer a = this._solveNLEByNewton( 1.19, new A111() );
